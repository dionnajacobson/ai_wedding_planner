...
### Request-scoped values for stateless tool executors

`AgentRunner`/`ToolOrchestrator` are shared, process-lifetime singletons (see `api/dependencies.py`), and their executors (`ToolOrchestrator.default()`) are constructed once at startup — so an executor can't hold per-request state (e.g. a `wedding_id`) in its constructor. Instead, put request-scoped values on the `ToolDefinition` itself, since a fresh one is built per agent construction (e.g. `SaveVendorDefinition(session_id=session_id)` in `WeddingAgent.chat()`). `ToolExecutor.execute(self, tool_call, tool_entry, runner)` receives the whole `ToolEntry` built by `prepare()` — `tool_entry.agent` is the child to run for agent-as-tool calls, and `tool_entry.definition` is the entry's `ToolDefinition`, whose subclass fields an executor reads directly (e.g. `SaveVendorExecutor.execute()` reading `tool_entry.definition.session_id`). `tool_entry` and `runner` are required on every executor's `execute()`, even if unused — there's no standalone/no-entry call path.
